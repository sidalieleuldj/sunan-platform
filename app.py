...
index 04426305ef41587f910e4506986425cceb8dbb99..82dcf698b004397b1cc64d7836ce8cf44c6817ba 100644
--- a/app.py
+++ b/app.py
@@ -1,284 +1,317 @@
...
+    """Nettoie le nom pour éviter les entrées bruitées (espaces, vide)."""
+    return " ".join(name.split())
+
+def validate_user_inputs(user_name, d_hours, p_ratio, projects, quality, orig, replies):
+    """Valide les données utilisateur et retourne des erreurs/warnings actionnables."""
+    errors, warnings = [], []
+
+    if not user_name:
+        errors.append("⚠️ الرجاء إدخال اسم مستخدم صالح قبل التحليل.")
+
+    if d_hours == 0 and p_ratio == 0 and projects == 0 and orig == 0 and replies == 0:
+        errors.append("⚠️ أدخل بيانات نشاطك الرقمي أولاً (كل القيم الحالية تساوي صفراً).")
+
+    if d_hours >= 10 and p_ratio >= 0.7:
+        warnings.append("ℹ️ ساعات التصفح مرتفعة جداً مقارنة بنسبة الإنتاج المعلنة. تأكد من دقة المعطيات.")
+
+    if projects == 0 and quality >= 4:
+        warnings.append("ℹ️ جودة عالية دون مشاريع منجزة: ربما يلزم تعديل أحد المؤشرين.")
+
...
+    st.image("https://cdn-icons-png.flaticon.com/512/2331/2331718.png", width=70)
+    user_name = st.text_input("اسم المستخدم", "مبادر")
+    clean_user_name = normalize_user_name(user_name)
+    
+    if st.button("🌓 تغيير المظهر"):
+        st.session_state.dark_mode = not st.session_state.dark_mode
+        st.rerun()
...
+
+if calc_btn:
+    errors, warnings = validate_user_inputs(clean_user_name, d_hours, p_ratio, projects, quality, orig, replies)
+    for msg in warnings:
+        st.warning(msg)
+
+    if errors:
...
+        if st.button("💾 حفظ في الأرشيف الحضاري"):
+            sheet = get_google_sheet()
+            if not clean_user_name or clean_user_name == "مبادر":
+                st.error("يرجى اختيار اسم مستخدم حقيقي قبل الحفظ.")
+            elif sheet:
+                sheet.append_row([clean_user_name, datetime.now().strftime("%Y-%m-%d"), str(eff), str(def_s), str(coh), diag])
+                st.balloons(); st.success("تم التوثيق!")
+
+# --- 7. RANKING RELATIF & HISTORIQUE (CORRIGÉ) ---
+st.markdown("---")
